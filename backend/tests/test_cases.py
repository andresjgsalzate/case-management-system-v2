...
def test_create_case_dto_requires_title():
    ...
    dto = CreateCaseDTO(title="Test case", priority_id="p1", service_item_id="svc1")
...
def test_create_case_dto_short_title_raises():
    ...
    with pytest.raises(Exception):
        CreateCaseDTO(title="X", priority_id="p1", service_item_id="svc1")  # mínimo 3 chars


def test_create_case_dto_requires_service_item_id():
    from backend.src.modules.cases.application.dtos import CreateCaseDTO
    with pytest.raises(Exception):
        CreateCaseDTO(title="Valid title", priority_id="p1")  # falta service_item_id


def test_create_case_dto_default_complexity():
    from backend.src.modules.cases.application.dtos import CreateCaseDTO
    dto = CreateCaseDTO(title="Valid title", priority_id="p1", service_item_id="svc1")
    assert dto.complexity == "simple"


def test_create_case_dto_complex_complexity():
    from backend.src.modules.cases.application.dtos import CreateCaseDTO
    dto = CreateCaseDTO(title="Valid title", priority_id="p1", service_item_id="svc1", complexity="complex")
